...
{
    if(it->second.type() == typeid(std::string))    //horible way of doing this, fix
    {
    ...
    int32_t v = std::any_cast<int32_t>(it->second);
...
void CreateStringLookup(Element* element, std::vector<std::string>& lookup)
{
    ...
    for(auto it=element->attributes.begin(); it != element->attributes.end(); it++)
    {
        ...
        if(it->second.type() == typeid(std::string) && it->first != "innerText" &&
           std::find(lookup.begin(),lookup.end(),std::any_cast<std::string>(it->second)) == std::end(lookup))
            lookup.push_back(std::any_cast<std::string>(it->second));
    }
...
void SaveAttribute(std::ostream& out, std::map<std::string, int16_t>& reverseLookup, std::map<std::string,std::any>::iterator it, Element* element)
{
    ...
    {
    case 0:
        Write<uint8_t>(out,std::any_cast<bool>(it->second));
        break;
    case 1:
        Write<uint8_t>(out,std::any_cast<int32_t>(it->second));
        break;
    case 2:
        Write<int16_t>(out,std::any_cast<int32_t>(it->second));
        break;
    case 3:
        Write<int32_t>(out,std::any_cast<int32_t>(it->second));
        break;
    case 4:
        Write<float>(out,std::any_cast<float>(it->second));
        break;
    case 5:
        Write<int16_t>(out, reverseLookup[std::any_cast<std::string>(it->second)]);
        break;
    case 6:
        WriteString(out,std::any_cast<std::string>(it->second));
        break;
    case  7:
        {
...
void LoadAttribute(std::istream& in, std::vector<std::string>& lookup, Element* element)
{
    ...
    {
    case 0:
        element->attributes[key]=std::make_any<bool>(Read<uint8_t>(in));
        break;
    case 1:
        element->attributes[key]=std::make_any<int32_t>(Read<uint8_t>(in));
        break;
    case 2:
        element->attributes[key]=std::make_any<int32_t>(Read<int16_t>(in));
        break;
    case 3:
        element->attributes[key]=std::make_any<int32_t>(Read<int32_t>(in));
        break;
    case 4:
        element->attributes[key]=std::make_any<float>(Read<float>(in));
        break;
    case 5:
        element->attributes[key]=std::make_any<std::string>(lookup[Read<int16_t>(in)]);
        break;
    case 6:
        element->attributes[key]=std::make_any<std::string>(ReadString(in));
        break;
    case 7:
        {
            ...
            element->attributes[key]=std::make_any<std::string>(v);
        }
        break;
    default:
        throw std::runtime_error("ERORR: Invalid data type");
